    ...
    <section className="debrief-panel" aria-labelledby="debrief-title">
      ...
      <div className="debrief-columns">
        ...
        <div className="debrief-side-column">
          ...
          <VitalsPanel vitals={state.vitals} title="Final vital signs" />
          <section className="debrief-section"><h3>Event summary</h3><dl className="debrief-event-grid">{events.map(([type, label]) => <div key={type}><dt>{label}</dt><dd>{logs.filter(entry => entry.event_type === type).length}</dd></div>)}</dl></section>
        </div>
      </div>
